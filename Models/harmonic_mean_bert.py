...
def calc_rouge(df):
    ''' 
    Calculate rouge scores and calculating harmonic mean of these scores 
    Args:
      df: dataframe
    Returns:
    ...
    #Here we calculate Harmonic mean of R-1 and R-2 scores
    m = np.array(scores_avg["rouge-1"]) * np.array(scores_avg["rouge-2"])
    s = np.array(scores_avg["rouge-1"]) + np.array(scores_avg["rouge-2"])
    sc = np.nan_to_num(2 * m / s)
...
def training():
    ...
    testing the model
    results are printed and also exported to a json file to output folder
    '''
    ...
    with open('./outputs/scores_eval_harmonic_bert.json', 'w') as fp:
    ...
    with open("./outputs/hm_bert_scores_test.json", "w") as fp:
